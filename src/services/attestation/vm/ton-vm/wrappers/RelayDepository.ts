...
export const ADDRESS_NONE = Address.parse(
  "EQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAM9c"
);

export type RelayDepositoryConfig = {
  owner: Address;
  allocator: Address;
  nonce: bigint;
};
...
export function relayDepositoryConfigToCell(
  config: RelayDepositoryConfig
): Cell {
  return beginCell()
...
export class RelayDepository implements Contract {
  constructor(
  ...
  static createFromAddress(address: Address) {
    return new RelayDepository(address);
  }
  ...
  static createFromConfig(
    ...
    const data = relayDepositoryConfigToCell(config);
    const init = { code, data };
    return new RelayDepository(contractAddress(workchain, init), init);
  }
  ...
  async sendTransfers(
    ...
    const transferCells = opts.requests.map((request) =>
      this.createTransferDataCell(request)
    );
  ...
  private createTransferDataCell(request: TransferRequest): Cell {
    ...
    const mainCell = beginCell()
      ...
      .storeAddress(request.to)
      .storeAddress(
        request.currencyType === CurrencyType.TON
          ? ADDRESS_NONE
    ...
    return mainCell.storeRef(signatureCell).endCell();
  }
  ...
  static async parseOutMessage(
    ...
    if (eventCode === 2290588233) {
      ...
      const eventJettonWallet = provider.open(
        JettonWallet.createFromAddress(Address.parse(jettonWallet))
      );
