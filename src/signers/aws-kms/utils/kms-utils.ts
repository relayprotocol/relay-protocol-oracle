...
const EcdsaPubKey = asn1.define<{
  algo: { a: string[]; b: string[] };
  pubKey: { data: Buffer };
}>("EcdsaPubKey", function (this: any) {
  this.seq().obj(
...
export function getEthereumAddress(publicKey: Buffer): `0x${string}` {
  ...
  const address = keccak256(new Uint8Array(pubKeyBuffer));
...
function recoverPubKeyFromSig(msg: Buffer, r: BN, s: BN, v: number): Promise<`0x${string}`> {
...
export async function signDigestHex(
  ...
  return signatureToHex(await _signDigest(digestString, kmsCredentials, address));
}
...
export async function signTransaction(
  ...
  const transactionSignature = await _signDigest(keccak256(serializedTx), kmsCredentials, address);
