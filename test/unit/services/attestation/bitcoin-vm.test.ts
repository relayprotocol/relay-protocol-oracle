...
import { DepositoryWithdrawalStatus, decodeWithdrawal, getDecodedWithdrawalId } from "@reservoir0x/relay-protocol-sdk";

// Test Bitcoin addresses and private keys
const testDepositoryAddress = "tb1qw508d6qejxtdg4y5r3zarvary0c5xw7kxpjzsx";
const testUserAddress = "tb1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3q0sl5k7";
  ...
  return {
    getChains: async () => ({
      bitcoin: {
        ...
        additionalData: {
          get esploraCompatibleApiUrl() { return mockEsploraUrl; },
        },
      },
    }),
    getChain: async () => ({
...
// Mock @reservoir0x/relay-protocol-sdk
...
const generateBitcoinTransaction = (
  ...
  return {
    ...
    vin: options.vin || [
      {
        txid: Buffer.from(randomHex(32).replace(/^0x/, ''), 'hex').reverse().toString('hex'),
        vout: 0,
        scriptSig: {
    ...
    confirmations: options.confirmations !== undefined ? options.confirmations : 3,
...
const generateBitcoinBlock = (
  ...
  return {
    ...
    time: options.time !== undefined ? options.time : Math.floor(Date.now() / 1000) - 3600,
...
describe("BitcoinVmAttestor", () => {
    ...
    it("should correctly parse Bitcoin deposit transaction", async () => {
      ...
      setupRpcMock({
        ...
        getBlock: jest.fn().mockReturnValue(
          generateBitcoinBlock(depositTx.blockhash)
        ),
      });
      ...
      expect(messages[0].result.currency).toBe("bc1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqmql8k8");
    });
    
    it("should throw error when transaction confirmations are insufficient", async () => {
      // Prepare test data
      const transactionId = randomHex(32);
      const depositTx = createDepositTransaction(transactionId, zeroHash, 100000000, 1); // Only 1 confirmation
  ...
  describe("getDepositoryWithdrawalMessage", () => {
    ...
    const setupWithdrawalTest = (options: {
      isSpent?: boolean;
      txMatches?: boolean;
      ...
      (decodeWithdrawal as jest.Mock).mockImplementation(() => decodedWithdrawal);
      (getDecodedWithdrawalId as jest.Mock).mockImplementation(() => "0x1234567890");
          ...
          inputs: [
            {
              witnessUtxo: {
                script: noAllocatorUtxos 
                  ? Buffer.from("00140000000000000000000000000000000000000000", "hex") // Different script
                  : Buffer.from("00147751e76e8199196d454941c45d1b3a323f1433bd6", "hex"),
              },
              partialSig: [
                {
                  pubkey: Buffer.from("0304c01563d46e38264283b99bb352b46e69bf132431f102d4bd9a9d8dab075e7f", "hex"),
                  signature: Buffer.from("3045022100f4d17785319488c32c4e3d339c5e8f317c94c4978e4b0641fb9cd4eacc89b0e802203c58f8c3ec9072a5e33a4c12b5641b3c5bca14047b7e1b6969d1d3c6e6210c1b01", "hex"),
                },
              ],
            },
      ...
      jest.spyOn(bitcoin.Psbt, "fromHex").mockImplementation(() => mockPsbt as any);
      ...
      if (multipleSpendingTxs) {
        ...
        (axios.get as jest.Mock).mockImplementation((url: unknown) => {
          ...
          const match = (url as string).match(/\/tx\/([a-f0-9]+)\/outspend\/(\d+)$/);
          if (match) {
            const [, , vout] = match;
        ...
        ins: [
          {
            witness: txMatches ? [
              ...
              Buffer.from("3045022100f4d17785319488c32c4e3d339c5e8f317c94c4978e4b0641fb9cd4eacc89b0e802203c58f8c3ec9072a5e33a4c12b5641b3c5bca14047b7e1b6969d1d3c6e6210c1b01", "hex"),
              Buffer.from("0304c01563d46e38264283b99bb352b46e69bf132431f102d4bd9a9d8dab075e7f", "hex"),
            ] : [
              // Different signature for non-matching case
              Buffer.from("3045022100aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa02203c58f8c3ec9072a5e33a4c12b5641b3c5bca14047b7e1b6969d1d3c6e6210c1b01", "hex"),
              Buffer.from("0304c01563d46e38264283b99bb352b46e69bf132431f102d4bd9a9d8dab075e7f", "hex"),
            ],
          },
          // Add a second input for multiple spending tx test
      ...
      jest.spyOn(bitcoin.Transaction, "fromHex").mockImplementation(() => mockTx as any);
      ...
      mockEsploraUrl = noEsploraUrl ? undefined : "https://blockstream.info/api";
    ...
    it("should throw error when no allocator UTXOs are detected", async () => {
      ...
      ).rejects.toThrow("No allocator UTXOs detected as part of the withdrawal request");
    });
