...
- erc20 tokens
  ...
  - the optional deposit id can be specified in one of two ways:
    - via a matching `Erc20Deposit(address from, address token, uint256 amount, bytes32 id)` event emitted on the escrow contract right after the `Transfer` event (eg. `logIndex + 1`)
