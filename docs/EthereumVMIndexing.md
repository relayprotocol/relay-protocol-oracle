...
- native tokens
  - the deposit is indexed based on the `#!solidity NativeDeposit(address from, uint256 amount, bytes32 id)` event emitted on the escrow contract (eg. one event results in one deposit tracked by the oracle)
- erc20 tokens
