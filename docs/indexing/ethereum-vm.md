...
- native tokens
  - the deposit is indexed based on the `RelayNativeDeposit(address from, uint256 amount, bytes32 id)` event emitted on the depository contract (eg. one event results in one deposit tracked by the oracle)
- erc20 tokens
